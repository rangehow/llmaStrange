...
def match_prefix(g_ngrams_list, doc_ngrams_list, step):
    ...
    for g_ngrams, doc_ngrams in zip(g_ngrams_list, doc_ngrams_list):
        ...
        # 不是的，因为在给定n的情况下，生成和docs的ngram不是任意的，生成到当前位置，docs是可以任意的
        print("---------------------------------------")
...
def make_kv_tupe(input_kv, step_length, accepted_step_length):
    '''
    截断到合法的accepted_step_length
    Args:
        input_kv: (batch_size, num_heads, sequence_length - 1, embed_size_per_head)
        step_length:
...
def make_past_key_values(past_key_values, step_length, accepted_step_length):
    ...
    Args:
        past_key_values: ( (batch_size, num_heads, sequence_length - 1, embed_size_per_head)*层数个)
        step_length:
        accepted_step_length:
...
def llma_generate(model, tokenizer, input_ids, gen_texts_ids, trigger_N, block_K, forced_decoding=False,
    ...
    :param input_ids:  一句话
    :param gen_texts_ids:
    :param trigger_N: 实际上并没有用上
    :param block_K: 往后匹配k个
    ...
    prepend_ids = input_ids.cuda()
    generate_ids = None
    # ( (batch_size, num_heads, sequence_length - 1, embed_size_per_head)*层数个)
    past_key_values = None
    ...
    step = 0
    while True:
        # prefix_n是匹配到的实际最长前缀长度（准备ngrams的时候是[n,maxn]，所以不是唯一的）
        # g_ngrams如果是forced decoding情况下就是有ref的，那就是返回一个prefix_n的ngram数组，否则的话也是list，但是里面元素只有一个。
        # doc_ngrams就是返回一个prefix_n的ngram数组
        prefix_n, g_ngrams, doc_ngrams = match_prefix(g_ngrams_list, doc_ngrams_list, step)
        if prefix_n > 0:
            copy_mode = True
            # 因为g_ngrams都是按照一个for循环送入的，step-prefix_n其实就是当前时刻的正确输出。如果不是forced是怎么处理的，不就一个吗？
            trigger_ngram = g_ngrams[step - prefix_n]
            # 因为是直接匹配到的，所以可以直接用trigger为键，这里其实有很多备选，不过llma说随便选一个，乐
            # TODO 这里可以vote？不要随机选第0个了……？怎么vote还可以进一步讨论。
            i, j = doc_ngrams[trigger_ngram][0]
            # 因为j是第j个ngram，其实起始位置就是j，被匹配到的实际上是doc[j:j+prefix_n]
            # 因为copied_ids和matched其实一定会有一个重叠交界词
        ...
        with torch.no_grad():
            ...
            # 因为维护了KV，所以输入只需要是最后一个词 :)
            prepend_ids = output_ids[:, -1:]
            if generate_ids is None:
                generate_ids = output_ids
            else:
                # output_ids的长度要么是1，要么是accepted_step_length
                # generate_ids一开始是空，后来是最开头的output_ids，然后继续拼接以后的生成。
                generate_ids = torch.concat([generate_ids, output_ids], dim=1)
            output_ids = output_ids.cpu().numpy()
...
def run_time_test(s_list, decoding_fn, model, tokenizer, trigger_N, block_K, append_docs=True, forced_decoding=False):
    ...
    Args:
        s_list: 就是json中的一条条数据，s[i]对应第i条json，更深层次可以用对应的键获取
        decoding_fn:
        model:
        tokenizer:
        trigger_N:
...
def prepare_batch_ngrams(batch, n, tokenizer, max_n=0):
    ...
    if 'result' in batch.keys():
        ...
        for l in range(n, max_n + 1):
            # 本身就是一个list，内容是['ngram1',..'ngramM']
            g_ngrams = get_batch_ngrams(gtokens, l)
            g_ngrams_list.append([l, g_ngrams])
        g_ngrams_list.reverse()
    else:
        gtokens = None
...
def main():
    ...
    if args.type == "base":
        ...
        total_time = run_batch_time_test(test_dataloader, base_generate, model, tokenizer, 1, 1,
                                         append_docs=args.append_docs,
                                         forced_decoding=args.forced_decoding)
        print(f"time for run batchllma:{total_time}")
    else:
        print("llma decoding")
        trigger_N = args.n
        block_K = args.k
